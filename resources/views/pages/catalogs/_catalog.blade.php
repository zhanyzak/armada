<section class="container catalog page-catalog__catalog mb-5">
    <div class="row">
        <aside class="filters catalog__filters col-12 col-md-3">
            <form action="{{ route('item',['slug'=>$item->slug]) }}" method="GET">
                ...
                @if(!$colors->isEmpty())
                <div class="filters__checkbox-group py-4 mb-4">
                    <p class="font-weight-semibold">Цвет</p>
                    <div class="custom-scrollbar default-skin" style="max-height: 340px">
                        <ul class="list-style-none mb-b p-0">
                            @foreach($colors as $color)
                                <li class="custom-control custom-checkbox mb-2"><!-- filter-apply  -->
                                    <input type="checkbox" name="color[]" value="{{ $color->slug }}" id="vendor-{{ $color->id }}" class="custom-control-input" @if(isset($_GET['color']) and in_array($color->slug,$_GET['color'])) checked @endif>
                                    <label for="vendor-{{ $color->id }}" class="custom-control-label">{{ $color->title_ru }}</label>
                                </li>
                            @endforeach
                        </ul>
                    </div>
                ...
                <div class="filters__checkbox-group py-4 mb-4">
                    <p class="font-weight-semibold">Тип материала</p>
                    <div class="custom-scrollbar default-skin" style="max-height: 340px">
                        <ul class="list-style-none mb-b p-0">
                            @foreach($materials as $material)
                                <li class="custom-control custom-checkbox mb-2"><!-- filter-apply  -->
                                    <input type="checkbox" name="material[]" value="{{ $material }}" id="vendor-{{ $material }}" class="custom-control-input" @if(isset($_GET['material']) and in_array($material,$_GET['material'])) checked @endif>
                                    <label for="vendor-{{ $material }}" class="custom-control-label">{{ $material }}</label>
                                </li>
                            @endforeach
                        </ul>
                    </div>
        ...
        <div class="catalog__content col-12 col-lg-9">
            <div class="catalog__header d-none d-lg-flex align-items-center justify-content-end">
                ...
                <select class="form-control w-auto bg-light border-0" id="select_filter" onchange="window.location.href=this.options[this.selectedIndex].value">
                    <option value="{{ route('item',['slug'=>$item->slug,'price'=>'down']) }}" @if(isset($_GET['price']) and $_GET['price'] == 'down') selected @endif>Цены по убыванию</option>
                    <option value="{{ route('item',['slug'=>$item->slug,'price'=>'up']) }}" @if(isset($_GET['price']) and $_GET['price'] == 'up') selected @endif>Цены по возрастанию</option>
                    <option value="{{ route('item',['slug'=>$item->slug]) }}" @if(!isset($_GET['price'])) selected @endif>Без сортировки</option>
                </select>
            </div>
